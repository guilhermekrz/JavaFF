...
//		this.state = this.getTemporalMetricInitialState();
	 ...
	 */
	public boolean isGoalReachable(Collection<Fact> goals)
	{
		for (Fact f : goals)
		{
...
//				a.setCondition(new And(modifiedPCs));
	...
	public Fact getFact(String f){
		for(Fact fact: this.getGroundedPropositions())
			if(fact.toString().equals(f))
				return fact;
	...
	public Set<Action> getActions(String[] actions){
		List<Action> groundActions = new ArrayList<Action>();
